...
function setup() {
  ...
  for (let i = 0; i < 10; i++) {
    slowParticles.push(new Particle("blue",random(width), random(height), Particle.slow, color(0, 0, 255)));
  }
  for (let i = 0; i < 10; i++) {
    fastParticles.push(new Particle("red", random(width), random(height), Particle.fast, color(255, 0, 0)));
  }
}
